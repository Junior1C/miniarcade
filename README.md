...
| `assets/js/*.js` суммарно | 44 КБ | критический путь каталога + витрина (редизайн 2026-09: ряды, сайдбар, drawer) |
| `assets/js/*` критический (без `stats-page.js`) | 36 КБ | главная: `stats-page.js` грузится только на `stats.html` (deferred), рост `main.js` виден отдельно |
