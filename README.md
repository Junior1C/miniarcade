...
2. Заполните `meta.json`:
   ```json
    ...
    `langs` — необязательный полный список языков (первый — основной,
    остальные — в подсказке бейджа): находит бот `qa-playable`
    (выбор языков в игре). Без `langs` бейдж рисуется по `lang`.
