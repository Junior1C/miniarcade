...
| `assets/js/*` критический (без `stats-page.js`, `settings.js`) | 40 КБ | главная: витрина и панель грузятся только по месту (deferred), рост `main.js` виден отдельно |
...
| `assets/css/main.css` | 26 КБ | тёмная неон-тема + панель оформления |
...
Шесть скриптов — ловят то, что не видят unit/e2e: нарушение контрактов игр, битые внутренние ссылки, секреты в коде, протухшие мосты, битые превью, неиграбельность. FAIL валит `npm run qa` (а значит pre-push и CI), WARN только шумит. Исключение: `qa-playable` требует playwright и сеть, поэтому живёт отдельно (`npm run qa:playable`) и в `npm run qa` не входит.
...
Кнопка `⚙` в топбаре открывает панель: 8 цветов (фон, поверхности, текст,
приглушённый, синий и фиолетовый акценты, глубокий фиолет, опасность) —
применяются сразу, запоминаются в `localStorage` этого браузера,
«Сбросить» возвращает стиль проекта. Кнопка «Скопировать стиль» отдаёт
JSON — пришлите его мейнтейнеру, и он вшьёт цвета в `main.css`
(`:root`) для всех. Панель грузится лениво (вне критического пути).
