...
const BUDGETS = [
  ...
  // Тёмная тема стора (топбар, сайдбар, ряды-карусели, обложки 3:2/1:1):
  // поднято 20→24 КБ редизайном 2026-09, дальше — только с обоснованием.
  { file: 'assets/css/main.css', maxBytes: 24 * 1024 },
  { file: 'assets/og.png', maxBytes: 100 * 1024 },
];

const JS_BUDGET = { dir: 'assets/js', maxBytes: 44 * 1024 };
...
// Поднято 24→28 честной сортировкой 2026-09, затем 28→36 редизайном
// (ряды, сайдбар жанров, drawer, карусели): разовый +8 КБ парсинга
// (~5 мс) за новую главную — цена зафиксирована, дальше только с разбором.
const JS_CRITICAL_MAX = 36 * 1024;
